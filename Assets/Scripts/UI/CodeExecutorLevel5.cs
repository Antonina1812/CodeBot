...
public class CodeExecutorLevel5 : MonoBehaviour
{
    ...
    private RobotController     _robot1; // синий
    private RobotController     _robot2; // жёлтый
    ...
    private IEnumerator ExecuteCommand(string cmd, string activeRobot)
    {
        if (activeRobot == "both")
        {
            ...
            if (_robot1 != null)
                StartCoroutine(RunSingle(_robot1, cmd, () => r1done = true));
            if (_robot2 != null)
                StartCoroutine(RunSingle(_robot2, cmd, () => r2done = true));
    ...
    // Находит закрывающий тег с учётом вложенности
